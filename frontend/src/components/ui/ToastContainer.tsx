...
const iconMap = {
  ...
  warning: (
    <svg className="h-5 w-5 text-tertiary-500" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div className="fixed top-16 right-4 z-[50] flex flex-col gap-2 max-w-sm">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={clsx(
            'flex items-center gap-3 rounded-lg border bg-bg-elevated px-4 py-3 shadow-lg animate-slide-up',
            toast.type === 'success' && 'border-primary-500/30',
            toast.type === 'error' && 'border-danger-500/30',
            toast.type === 'info' && 'border-secondary-500/30',
            toast.type === 'warning' && 'border-tertiary-500/30',
          )}
        >
